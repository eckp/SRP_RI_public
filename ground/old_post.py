...
states = ['OFF', 'IDLE', 'PREPARED', 'ARMED', 'LAUNCHED', 'DEPLOYED', 'LANDED']
fullscreen = 1
    ...
    for dd in str_data:
        ...
        if not dd[0].startswith('#'):
            if len(dd) == 2:
                ...
                dat.append(float(dd[0]))
                dat.append([float(d) for d in dd])
            data.append(dat)
    return data
...
def calculate_alt_vv(baro_data):
    pressure_raw = [b[1]/40.96 for b in baro_data]
    global p0
    p0 = np.mean(pressure_raw[:100])
...
def calculate_acc_g(acc_data):
    acc_raw = [[aa*0.122/1000 for aa in a[1]] for a in acc_data]  # conversion from LSB to g's
    return acc_raw
...
def calculate_gyro_dps(gyro_data):
    ...
    gyro_raw = [[gg*1 for gg in g[1]] for g in gyro_data]  # conversion from LSB to dps
    return gyro_raw

def calculate_mag_gaus(mag_data):
    mag_raw = [[mm/6842 for mm in m[1]] for m in mag_data]  # conversion from LSB to gauss
    return mag_raw
...
def calculate_heading(mag_list):
    ...
    mag = np.array([m[1] for m in mag_list])
    times = np.array([m[0] for m in mag_list])
...
def get_state_transitions(log):
    ...
    for line in loglines:
        ...
        for word in words[::-1]:
            if word in states:
                state_transitions.append([words[0], word])
                break
...
if __name__ == '__main__':
    while True:
        ...
        for i, name in enumerate(sensors):
            ...
            ax.set_title(name)
            plot([d[0] for d in sensors[name]], [d[1] for d in sensors[name]], plotter=ax)
